...
import { listMySpots, createSpot, updateSpot, deleteSpot } from '../services/parking';
...
export default function ProviderDashboard() {
  ...
  const setActiveTab = (tab) => navigate(`/provider/${tab}`);
  ...
  const handleCreateSpot = async (e) => {
    ...
    try {
      ...
      if (editingSpotId) {
        await updateSpot(editingSpotId, payload);
        showToast('🏠 Parking Space updated successfully!');
      } else {
  ...
  const handleEditClick = (spot) => {
    ...
    setForm({
      ...
      latitude: spot.location?.latitude?.toString() || '23.0225',
      longitude: spot.location?.longitude?.toString() || '72.5714',
      images: (spot.images || []).join(', '),
      parkingType: spot.parkingType || 'driveway',
      totalSlots: spot.totalSlots?.toString() || '5',
      availableFrom: spot.openingTime || '08:00',
      availableTill: spot.closingTime || '22:00',
      pricePerHour: spot.hourlyPrice?.toString() || '40',
      dynamicPricing: true
    });
    setAddStep(1);
    setActiveTab('add-space');
  };
    ...
    < div className = "flex items-start justify-between flex-wrap gap-4" >
      <div>
        <h2 className="text-3xl font-extrabold tracking-tight text-[var(--text-primary)]">
          ...
          {activeTab === 'add-space' && (editingSpotId ? 'Edit Parking Space' : 'Onboard New Space')}
        ...
        <p className="text-[var(--text-muted)] mt-1">
          ...
          {activeTab === 'add-space' && (editingSpotId ? 'Update your space details and configurations.' : 'Onboard your unused garage or driveway in 5 easy steps.')}
      ...
      </div>
{
  activeTab === 'dashboard' && (
    <Button icon={Plus} onClick={() => {
      setEditingSpotId(null);
      setForm({
        title: '', description: '', address: '', latitude: '23.0225', longitude: '72.5714',
        images: '', parkingType: 'driveway', totalSlots: '5', availableFrom: '08:00',
        availableTill: '22:00', pricePerHour: '40', dynamicPricing: true
      });
      setActiveTab('add-space');
    }}>
      List a New Space
              ...
              <div key={spot._id} className="glass-panel overflow-hidden group hover:shadow-2xl hover:scale-[1.01] transition-all flex flex-col justify-between">
                ...
                <div className="p-5 pt-0 flex gap-2">
                  <button
                    ...
                    className="flex-1 py-3 text-sm font-bold text-[var(--text-primary)] border border-[var(--border-color)] rounded-xl hover:bg-[var(--bg-card-hover)] transition-colors"
                  >
                    Edit
                  </button>
        ...
        <form onSubmit={handleCreateSpot} className="space-y-6">
          ...
          {addStep === 3 && (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
              ...
              {/* Upload Drop Zone */}
              <div
                className="relative border-2 border-dashed border-indigo-500/40 rounded-2xl p-8 text-center cursor-pointer hover:border-indigo-400 hover:bg-indigo-500/5 transition-all group"
                onClick={() => document.getElementById('parking-img-input').click()}
                ...
                onDrop={e => {
                  ...
                  files.forEach(file => {
                    ...
                    reader.onload = ev => {
                      setForm(prev => ({ ...prev, images: prev.images ? prev.images + ',' + ev.target.result : ev.target.result }));
                    };
              ...
              >
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-[var(--text-primary)] text-sm">Drag & drop photos here</p>
                    <p className="text-xs text-[var(--text-muted)] mt-0.5">or click to browse from your device</p>
                  </div>
                  <button
                    type="button"
                    className="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition-all shadow-lg shadow-indigo-500/30"
                  >
                    📷 Upload Photos
                  </button>
                  <p className="text-[10px] text-[var(--text-muted)]">PNG, JPG, WEBP supported · Multiple files allowed</p>
                </div>
              </div>

              {/* Image Previews */}
              {form.images && form.images.split(',').filter(u => u.trim()).length > 0 && (
                <div className="grid grid-cols-3 gap-3">
                  {form.images.split(',').filter(u => u.trim()).map((url, idx) => (
                    <div key={idx} className="relative group rounded-xl overflow-hidden border border-[var(--border-color)] aspect-square bg-[var(--bg-card)]">
                      <img src={url.trim()} alt={`Preview ${idx + 1}`} className="w-full h-full object-cover" />
                      <button
                        type="button"
                        onClick={() => {
                          const arr = form.images.split(',').filter(u => u.trim());
                          arr.splice(idx, 1);
                          setForm(prev => ({ ...prev, images: arr.join(',') }));
                        }}
                        className="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-red-500 text-white text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity font-bold"
                      >✕</button>
              ...
              <details className="group">
                <summary className="text-xs text-indigo-400 cursor-pointer hover:text-indigo-300 font-semibold select-none">
                  + Or paste image URLs instead
                </summary>
                <div className="mt-2">
                  <textarea
                    ...
                    className="input-premium w-full text-sm px-4 py-3 h-20"
                  />
                  <div className="flex gap-2 items-center mt-2">
                    <label className="cursor-pointer bg-indigo-500/10 text-indigo-400 px-4 py-2 rounded-lg font-bold text-xs hover:bg-indigo-500/20 transition flex items-center gap-2">
                      ...
                      <input type="file" className="hidden" accept="image/*" onChange={async (e) => {
                        if (e.target.files && e.target.files[0]) {
                          ...
                          try {
                            ...
                            if (res.data.success) {
                              const newUrl = import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace('/api', '') + res.data.image : 'http://localhost:5006' + res.data.image;
                              setForm(f => ({ ...f, images: f.images ? f.images + ', ' + newUrl : newUrl }));
                    ...
                    <span className="text-[10px] text-[var(--text-muted)]">Upload files or paste absolute URLs.</span>
                  </div>
                </div>
              </details>
            </motion.div>
